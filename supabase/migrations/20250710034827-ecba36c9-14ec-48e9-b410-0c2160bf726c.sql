-- Create CRD-specific asset tables for MediaManager (fixed foreign keys)
...
CREATE TABLE IF NOT EXISTS crd_frames (
  ...
  creator_user_id UUID, -- References auth user ID directly
...
CREATE TABLE IF NOT EXISTS media_assets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID, -- References auth user ID directly
...
CREATE INDEX IF NOT EXISTS idx_crd_frames_creator_user_id ON crd_frames(creator_user_id);
...
CREATE INDEX IF NOT EXISTS idx_crd_elements_creator_user_id ON crd_elements(creator_user_id);
...
CREATE INDEX IF NOT EXISTS idx_crd_materials_creator_user_id ON crd_materials(creator_user_id);
...
CREATE INDEX IF NOT EXISTS idx_crd_templates_creator_user_id ON crd_templates(creator_user_id);
